...
public interface ChallengeUserRepository extends JpaRepository<ChallengeUser, Long> {
    ...
    @Query(value = "SELECT c.id, c.title, c.info, c.num, c.deadline  FROM Challenge c LEFT OUTER JOIN ChallengeUser cu on c.id = cu.challenge_id\n" +
            "WHERE c.id NOT IN (SELECT cu.challenge_id FROM ChallengeUser cu WHERE cu.user_id = :userId) ORDER BY c.num DESC;", nativeQuery = true)
