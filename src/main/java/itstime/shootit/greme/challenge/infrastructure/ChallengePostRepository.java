...
public interface ChallengePostRepository extends JpaRepository<ChallengePost, Long> {
    ...
    @Query(value = "SELECT p.id, p.image FROM challenge_post cp LEFT OUTER JOIN challenge_user cu on cu.challenge_id = cp.challenge_id LEFT OUTER JOIN post p on p.id = cp.post_id WHERE cu.challenge_id = :challenge_id AND p.status = true", nativeQuery = true)
